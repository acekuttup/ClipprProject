import { ChevronRight, Crown, LogOut, MapPin, Bell, CreditCard, FileText, User, Trash2 } from "lucide-react";
import { useClipperStore, PAYMENT_METHOD_LABELS } from "@/lib/clipper-store";

export function SettingsScreen({ onAuth, onPremium }: { onAuth: () => void; onPremium: (f?: string) => void }) {
  ...
  return (
    <div className="space-y-4 pb-24">
      ...
      <div className="card-luxe p-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="text-xs text-muted-foreground">{store.user?.email ?? "Not signed in"}</div>
          </div>
          {isPremium && (
      ...
      <Section label="Trip Detection" icon={<MapPin className="h-4 w-4" />}>
        <Row label="Work Location" value={store.settings.workLocationName ?? "Tap to set"} premium={!isPremium} onClick={() => onPremium("Auto trip detection")} />
      </Section>
        ...
        <Row
          ...
          value={
            store.boothRentConfig
              ? `$${store.boothRentConfig.amount} ${store.boothRentConfig.frequency}`
              : "Not configured"
          }
        />
      </Section>
      ...
      <Section label="Connected Accounts" icon={<CreditCard className="h-4 w-4" />}>
        <Row label="Cash App" value="Not connected" premium={!isPremium} onClick={() => onPremium("Bank-sync")} />
        <Row label="Venmo" value="Not connected" premium={!isPremium} onClick={() => onPremium("Bank-sync")} />
        <Row label="Zelle" value="Not connected" premium={!isPremium} onClick={() => onPremium("Bank-sync")} />
      </Section>

      <Section label="Tax Information" icon={<FileText className="h-4 w-4" />}>
        <Row label="Business Code" value={store.settings.businessCode} />
        <Row label="Mileage Rate" value={`$${store.settings.mileageRatePerMile.toFixed(2)}/mi (${store.settings.mileageRateYear})`} />
      </Section>
      ...
      <Section label="Payment Methods" icon={<CreditCard className="h-4 w-4" />}>
        <div className="px-4 pb-3 pt-1 text-xs text-muted-foreground">
          {store.settings.preferredPaymentMethods.map((m) => PAYMENT_METHOD_LABELS[m]).join(" · ") || "None selected"}
        </div>
      </Section>

      <Section label="Notifications" icon={<Bell className="h-4 w-4" />}>
        <Toggle label="Booth rent reminders" on={store.settings.boothRentNotificationsOn} onClick={() => toggle("boothRentNotificationsOn")} />
        <Toggle label="Review queue badge" on={store.settings.reviewQueueBadgeOn} onClick={() => toggle("reviewQueueBadgeOn")} />
        <Toggle label="Weekly receipt prompt" on={store.settings.weeklyReceiptPromptOn} onClick={() => toggle("weeklyReceiptPromptOn")} />
      </Section>
...
function Row({ label, value, premium, onClick }: { label: string; value: string; premium?: boolean; onClick?: () => void }) {
  return (
    <button
      onClick={onClick}
...
function Toggle({ label, on, onClick }: { label: string; on: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className="flex w-full items-center justify-between px-4 py-3 text-left tap-highlight">
      <span className="text-sm">{label}</span>
      <span className={`relative h-6 w-11 rounded-full transition ${on ? "bg-brass" : "bg-muted"}`}>
        <span className={`absolute top-0.5 h-5 w-5 rounded-full bg-background transition-all ${on ? "left-5" : "left-0.5"}`} />
      </span>
    </button>
  );
}
